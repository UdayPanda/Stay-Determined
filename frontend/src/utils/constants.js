export const HOST = "https://stay-determined-backend.onrender.com";

export const AUTH_ROUTE  = "api/auth";
export const SIGNUP_ROUTE = `${AUTH_ROUTE}/signup`;
export const LOGIN_ROUTE = `${AUTH_ROUTE}/login`;
export const LOGIN_ME_ROUTE = `${AUTH_ROUTE}/me`;
export const LOGOUT_ROUTE = `${AUTH_ROUTE}/logout`;
export const LOGIN_ROUTE_GOOGLE = `${AUTH_ROUTE}/google`;
export const FORGOT_PASSWORD_ROUTE = `${AUTH_ROUTE}/forgot-password`;
export const SEARCH_ROUTE = `${AUTH_ROUTE}/search`;

export const TODO_ROUTE = "api/todos";
export const ADD_TODO = `${TODO_ROUTE}/add`;
export const GET_TODOS = `${TODO_ROUTE}/get`;
export const DELETE_TODO = `${TODO_ROUTE}/delete`;
export const UPDATE_TODO = `${TODO_ROUTE}/update`;

export const NOTE_ROUTE = "api/notes";
export const ADD_NOTES = `${NOTE_ROUTE}/add`;
export const GET_NOTES = `${NOTE_ROUTE}/get`;
export const DELETE_NOTE = `${NOTE_ROUTE}/delete`;

export const EXPANSE_ROUTE = "api/expanse";
export const ADD_EXPANSE = `${EXPANSE_ROUTE}/add`;
export const GET_EXPANSE = `${EXPANSE_ROUTE}/get`;
export const GET_BALANCE = `${EXPANSE_ROUTE}/balance`;
export const DELETE_EXPANSE = `${EXPANSE_ROUTE}/delete`;
export const BULK_UPLOAD_EXPANSE = `${EXPANSE_ROUTE}/bulk`;

export const PLAN_ROUTE = "api/plans";
export const ADD_PLAN = `${PLAN_ROUTE}/add`;
export const GET_PLAN = `${PLAN_ROUTE}/get`;
export const UPDATE_PLAN = `${PLAN_ROUTE}/update`;
export const DELETE_PLAN = `${PLAN_ROUTE}/delete`;